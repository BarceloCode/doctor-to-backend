...
async function retrive() {
  try {
    const Machine = await MachineSchema.find();
    ...
    return { message: "Error", error: "Machines not found" };
  }
}

async function create(req) {
  try {
    const Machine = await MachineSchema.create({
      name: req.body.name,
...
async function update(req) {
  try {
    const finduser = await MachineSchema.findOne({
    ...
    const result = await MachineSchema.updateOne(finduser, update);
    if (result) {
